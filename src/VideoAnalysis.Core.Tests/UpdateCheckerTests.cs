...
public class UpdateCheckerTests
{
    ...
    public async Task CheckForUpdateAsync_ReturnsUpdate_WhenLatestReleaseIsNewer()
    {
        ...
        {
          "tag_name": "v0.3.0",
          "html_url": "https://github.com/canonn-science/VideoAnalysis/releases/tag/v0.3.0",
          "prerelease": false,
          "assets": [
            { "name": "VideoAnalysisLab-0.3.0-win-x64-Setup.exe", "browser_download_url": "https://example.com/setup.exe" }
          ]
        }
        """;
        using var checker = MakeChecker(json);

        var update = await checker.CheckForUpdateAsync("canonn-science", "VideoAnalysis", new Version(0, 2, 0));

        Assert.NotNull(update);
        Assert.Equal(new Version(0, 3, 0), update!.Version);
        Assert.Equal("https://example.com/setup.exe", update.InstallerDownloadUrl);
        Assert.Equal("VideoAnalysisLab-0.3.0-win-x64-Setup.exe", update.InstallerFileName);
    }
    ...
    public async Task CheckForUpdateAsync_ReturnsNull_WhenCurrentVersionIsUpToDate()
    {
        ...
        {
          "tag_name": "v0.2.0",
          "html_url": "https://github.com/canonn-science/VideoAnalysis/releases/tag/v0.2.0",
          "prerelease": false,
          "assets": [
            { "name": "VideoAnalysisLab-0.2.0-win-x64-Setup.exe", "browser_download_url": "https://example.com/setup.exe" }
          ]
        }
        """;
        using var checker = MakeChecker(json);

        var update = await checker.CheckForUpdateAsync("canonn-science", "VideoAnalysis", new Version(0, 2, 0, 0));
    ...
    public async Task CheckForUpdateAsync_ReturnsNull_WhenNoInstallerAssetPresent()
    {
        const string json = """
        {
          ...
          "prerelease": false,
          "assets": [
            { "name": "VideoAnalysisLab-v0.3.0-win-x64.zip", "browser_download_url": "https://example.com/zip" }
          ]
        }
        """;
        using var checker = MakeChecker(json);
